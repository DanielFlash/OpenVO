#include "BaseTrainerDQN.h"

BaseTrainerDQN::BaseTrainerDQN(BaseModelDQN* model, const double gamma, torch::optim::Adam* optimizerAdam, torch::Device* device)
...
void BaseTrainerDQN::trainStep(std::vector<std::vector<int>> state, std::vector<std::vector<int>> action,
	...
	for (int i = 0; i < done.size(); i++) {
		...
		if (!done[i]) {
			auto QNew = rewardT[i][0] + m_gamma * torch::max(m_Model->forward(nextStateT[i])).to(*m_device);
		}
